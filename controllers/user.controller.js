...
const Sitedata = require('../json/data.json');
...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (userNameExists) {
    res.status(400).json({ message: 'Нэвтрэх нэр бүртгэлтэй байна' });
  }
  if (phone) {
    if (userPhoneExists) {
      res.status(400).json({ message: 'Дугаар бүртгэлтэй байна' });
    }
  }
  if (email) {
    if (userEmailExists) {
  ...
  if (user) {
    ...
    res.status(400).json({ message: 'Алдаа: Дахин оролдоно уу!' });
  }
});
...
const userLogin = asyncHandler(async (req, res) => {
  ...
  if (!user) {
    res.status(400).json({ message: 'Хаяг олдсонгүй' });
  }
  if (user && (await bcrypt.compare(password, user.password))) {
    ...
    res.status(400).json({ message: 'Нууц үг буруу' });
  }
});
...
const getAllUser = asyncHandler(async (req, res) => {
  ...
  if (userRole.super_admin && userRole.admin !== user.role) {
    res.status(400).json({ message: 'Нэвтрэх эрхгүй' });
  } else {
...
const updateUser = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    (req.user.role === userRole.super_admin && userRole.admin)
  ) {
    ...
    if (roleDetect !== '0') {
      if (role) {
        res.status(401).json({ message: 'Эрхгүй' });
      }
    }
    try {
      ...
      res.status(401).json({ message: 'Хэрэглэгч зөвшөөрөлгүй' });
    }
  } else {
    res
      .status(403)
      .json({ message: 'Та зөвхөн өөрийн хаягаа засах боломжтой!' });
  }
});
...
const deleteUser = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    res
      .status(403)
      .json({ message: 'Та зөвхөн өөрийн хаягаа устгах боломжтой!' });
  }
});
